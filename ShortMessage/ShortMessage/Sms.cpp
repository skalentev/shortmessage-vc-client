...
//	TRACE("%s",pDst);
	...
	if(pSrc->TP_COUNT == 1){
		...
		buf[0] = 0x51;					// 是发送短信(TP-MTI=01)，TP-VP用相对格式(TP-VPF=10)
		buf[1] = 0;						// TP-MR=0
		buf[2] = (char)nLength;			// 目标地址数字个数(TP-DA地址字符串真实长度)
		buf[3] = 0x91;					// 固定: 用国际格式号码
		nDstLength += gsmBytes2String(buf, &pDst[nDstLength], 4);		// 转换4个字节到目标PDU串
		...
		buf[5] = 0;						// 一字节分拆
		buf[6] = 0x03;					// 剩余报头长度
		buf[7] = pSrc->TP_ID;			// 短信ID
		buf[8] = pSrc->TP_COUNT;		// 短信总数
		buf[9] = pSrc->TP_CURRENT;		// 当前第N条
		if(pSrc->TP_DCS == GSM_7BIT)	
		{
			// 7-bit编码方式
			buf[3] = nLength + 6;		// 编码前长度
			nLength = gsmEncode7bit(pSrc->TP_UD, &buf[10], nLength+1) + 10;	// 转换TP-DA到目标PDU串
		}
		else if(pSrc->TP_DCS == GSM_UCS2)
		{
			// UCS2编码方式
			buf[3] = gsmEncodeUcs2(pSrc->TP_UD, &buf[10], nLength) + 6;	// 转换TP-DA到目标PDU串		
			nLength = buf[3] + 4;		// nLength等于该段数据长度
		}
		else
		{
			// 8-bit编码方式
			buf[3] = gsmEncode8bit(pSrc->TP_UD, &buf[10], nLength) + 6;	// 转换TP-DA到目标PDU串
			nLength = buf[3] + 4;		// nLength等于该段数据长度
		}
	}
